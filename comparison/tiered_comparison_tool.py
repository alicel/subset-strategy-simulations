...
class TieredComparisonAnalyzer:
    ...
    def save_comparison_csv(self, comparisons: List[TieredComparisonResult], output_file: str, exec1_name: str = None, exec2_name: str = None):
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            fieldnames = [
                'Migration_ID',
                'Exec1_Execution_Time', 'Exec2_Execution_Time', 'Execution_Time_Ratio_2_1', 'Execution_Time_Ratio_1_2',
                'Exec1_Workers', 'Exec2_Workers', 'Worker_Ratio_2_1', 'Worker_Ratio_1_2',
                'Exec1_CPUs', 'Exec2_CPUs', 'CPU_Ratio_2_1', 'CPU_Ratio_1_2',
                'Exec1_CPU_Time', 'Exec2_CPU_Time', 'CPU_Time_Ratio_2_1', 'CPU_Time_Ratio_1_2',
    ...
    def generate_comparison_report(self, comparisons: List[TieredComparisonResult], exec1_name: str = None, exec2_name: str = None, exec1_only: Set[str] = None, exec2_only: Set[str] = None) -> str:
        ...
        lines.append(f"{'ID':<12} {exec1_short:<10} {exec2_short:<10} {'2/1':<5} {'1/2':<5} {exec1_short:<8} {exec2_short:<8} {'2/1':<5} {'1/2':<5} {exec1_short:<8} {exec2_short:<8} {'2/1':<5} {'1/2':<5} {exec1_short:<10} {exec2_short:<10} {'2/1':<5} {'1/2':<5}")
        lines.append("-" * 125)
        ...
        for comp in comparisons:
            ...
            lines.append(f"{comp.migration_id:<12} {exec1_time:<10} {exec2_time:<10} {time_ratio_21:<5} {time_ratio_12:<5} "
                        f"{exec1.total_workers:<8} {exec2.total_workers:<8} {worker_ratio_21:<5} {worker_ratio_12:<5} "
                        f"{exec1.total_cpus:<8} {exec2.total_cpus:<8} {cpu_ratio_21:<5} {cpu_ratio_12:<5} "
                        f"{exec1_cpu_time:<10} {exec2_cpu_time:<10} {cpu_time_ratio_21:<5} {cpu_time_ratio_12:<5}")
        ...
        if total_exec1_time > 0:
            lines.append(f"  Exec2/Exec1: {total_exec2_time/total_exec1_time:.2f} (efficiency: <1.0 = exec2 faster)")
            lines.append(f"  Exec1/Exec2: {total_exec1_time/total_exec2_time:.2f} (speedup: >1.0 = exec2 faster)")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Compare specific tiered execution runs with comparison name
  ...
  python tiered_comparison_tool.py --exec1-path tiered/output/test_new_5 --exec2-path tiered/output/test_new_6 --comparison-exec-name my_tiered_analysis
    ...
    parser.add_argument('--comparison-exec-name', '-c',
                       help='Name for this comparison analysis (creates output/tiered_comparison/{name}/ directory)')
    ...
    try:
        ...
        if args.output:
            ...
            analyzer.save_comparison_csv(comparisons, args.output, exec1_name, exec2_name)
        elif args.output_csv and args.comparison_exec_name:
            # Generate organized output
            output_dir = f"output/tiered_comparison/{args.comparison_exec_name}"
